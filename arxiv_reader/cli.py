...
ARXIV_URL = "http://export.arxiv.org/api/query?search_query=(%(categories)s) AND lastUpdatedDate:[%(start)s TO %(end)s]&max_results=800"  # noqa: E501
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.date is None:
        ...
        if date is None:
            logger.error("Could not parse %s as date.", args.date)
            return 1
        else:
